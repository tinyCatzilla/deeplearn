...
#   metrics = Metrics(...) # metrics object, note that i dont know what this is yet
#   output_path = "output"
#   save = True
#   verbose = True
...
class TestTrain:
    ...
    def fit(self, metric='pnsr'):
        ...
        for epoch in range(self.max_epochs):
            ...
            # print results
            if self.verbose:
                print_msg = f"Epoch: {epoch + 1} Train Loss: {train_loss:.4f} Val Loss: {val_loss:.4f}"
                if train_metrics is not None:
                    print_msg += f" Train Metrics: {train_metrics}"
                if val_metrics is not None:
                    print_msg += f" Val Metrics: {val_metrics}"
                print_msg += f" Time: {(time.time() - epoch_start_time) / 60:.2f} min"
                print(print_msg)
                self.print_pics()
        ...
        # print results
        print(f'Finished experiment {self.exp_prefix} taking {(time.time() - start_train) / 60:.2f} min')
